...
- 已实现账号密码认证、可撤销服务端会话、管理员创建/重置普通账号、强制首次改密、用户禁用和唯一系统管理员保护；随机密码只在创建或重置响应中显示一次，首次改密不重复要求当前密码。
- 已实现默认私有的数据集/模型项目、永久 owner、editor/viewer 成员管理、固定原子权限、系统管理员隐式全权访问和元数据乐观并发控制。
- 已实现模型项目范围内的模板与训练任务、系统模板只读，以及训练/自动标注等跨项目操作的输入资源消费权限校验。
...
- 已移除早期临时模型项目及其测试数据，改为内置“YOLO11目标检测官方模型”；普通账号自动只读，管理员可维护项目内模型但不能删除内置项目。YOLO `.pt` 模型入库、模型项目/模型二级选择、单张复核草稿和批量自动标注任务已实现；批量运行期间视频状态提示并锁定标注写入。
