...
- 默认多用户、可切换单用户的账号密码认证。
- 唯一受保护的系统管理员创建普通账号、一次性展示并可复制随机初始密码、强制首次改密，并可禁用/启用账号或重新生成密码；首次改密已由初始密码登录完成身份校验，只需输入新密码；系统不开放注册。
- 12 小时空闲/7 天绝对有效期的 HttpOnly 服务端会话，以及浏览器同源写保护。
- 默认私有的数据集与模型项目、永久所有者以及 editor/viewer 成员权限。
- 固定原子权限驱动项目读写；系统管理员无需成员授权即可全权管理所有项目，普通用户仍按项目隔离。
...
- 普通用户可创建私有归档模型项目并授权 editor/viewer；owner/editor 可编辑、导入 `.pt` YOLO 和执行模型操作，viewer 仅查看和下载，系统管理员隐式拥有全部权限。工作区默认提供显示 ID 为 `000001` 的“YOLO11目标检测官方模型”：普通账号自动只读，管理员可维护项目和模型，但不能删除该内置项目；历史“临时模型项目”及其数据会由迁移清理。
- YOLO Detect 超参数模板归属模型项目；项目 owner/editor 可创建、派生和编辑，viewer 只读，系统模板全局可读且不可修改。核心参数与扩展参数支持表单/RAW YAML 双向编辑。
- 每个训练草稿创建时即关联私有训练模型项目；任务访问继承项目权限，并支持单模型、单 GPU 串行和多 GPU 自定义序列。
...
- 自动标注支持项目标签、`All` 全类别及临时英文提示词；X-AnyLabeling 文本提示模型会将 `All` 展开为项目全部已启用英文标签，项目无启用标签时需先新增、启用或手工输入类别；只为实际检出的新类别创建项目标签。
