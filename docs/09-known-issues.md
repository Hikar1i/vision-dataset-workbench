...
状态：应用基础、视频导入、采样、抽帧、筛帧、项目标签、在线矩形标注、YOLO 模型入库、模型项目基础和本地/远程自动标注已实现；下表同时记录当前限制、遗留风险和已批准方案。

## 当前项目限制

- 当前已能初始化、登录、管理及归档删除项目、管理标签、导入本地或远程视频、配置/执行采样、筛帧、手动矩形标注、登记 YOLO 推理模型、按模型项目或 X-AnyLabeling Server 运行单张/批量自动标注及导出数据集；模型项目管理页面、在线训练、在线大模型、归档恢复和所有权转移尚未提供。
...
- 前端全量导入 Element Plus、Konva 等依赖，当前生产主 JavaScript chunk 约 1.37 MB；业务页面增加后应按测量结果拆包或按需导入。
