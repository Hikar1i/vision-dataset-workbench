...
状态：新项目尚无部署产物；本文记录遗留风险和发布约束。

## 目标部署

首要部署拓扑、操作系统、容器平台、数据库和任务服务均为 `Unknown`。在这些信息确认前，不提供假定可用的生产命令。
...
单机部署可以把组件放在同一主机，但 API 与 Worker 的职责和进程生命周期仍应分开。
...
- 原始媒体、抽帧和导出产物是否全部备份取决于存储成本和可重建策略，目前为 `Unknown`。
