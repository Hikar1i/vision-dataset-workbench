...
当前后端 86 项测试覆盖健康检查、路径逃逸、工作区定位、一次性口令、五版迁移、初始化回滚、认证会话生命周期、模式切换、注册审批、项目权限、媒体解析、视频导入、三种采样算法、采样/帧 API、帧版本冲突、任务租约/并发/取消/重试、抽帧发布与重采样保护。前端 28 项测试覆盖路由、初始化与认证、用户/项目管理、视频工作区、认证文件选择器、导入确认、采样配置、帧浏览筛选以及任务抽屉轮询与操作。
