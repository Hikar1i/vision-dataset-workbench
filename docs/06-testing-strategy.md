...
当前后端 50 项测试覆盖健康检查、路径逃逸、工作区定位、一次性口令、三版迁移、初始化回滚、认证会话生命周期、模式切换、改密撤销、注册审批、用户状态机、离线管理员重置、项目隔离、成员角色和乐观并发。前端 19 项测试覆盖路由、目录选择器、初始化、登录、注册、改密、退出、管理员用户操作、项目列表/创建以及项目设置/成员权限差异。

项目权限还通过隔离工作区中的真实 Uvicorn 进程冒烟验证：多用户隔离、editor 修改、viewer 拒写、过期版本 409、多用户管理员不越权，以及重启到 single 模式后的管理员等效 owner 访问均通过。媒体主流程端到端测试在对应页面和 Worker 实现后引入；不先设置脱离实际风险的覆盖率数字。
