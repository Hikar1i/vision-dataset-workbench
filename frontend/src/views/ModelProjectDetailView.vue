...
async function load() {
  ...
  error.value = ''
  try {
    const [nextProject, nextModels] = await Promise.all([
      getModelProject(projectId), listModelProjectModels(projectId),
    ])
...
async function saveProject() {
  ...
  saving.value = true
  try {
    project.value = await updateModelProject(projectId, name.value, description.value, project.value.version)
...
async function importModel() {
  ...
  try {
    const registered = await registerInferenceModel(projectId, importName.value, sourcePath.value, importDescription.value)
...
<template>
  <main class="content-page model-detail-page">
    ...
    <div v-loading="loading" class="content-body">
      ...
      <section v-if="project" class="project-summary"><div><span>项目类型</span><strong>{{ project.series_type === 'training' ? '训练' : '归档' }}</strong></div><div><span>权限</span><strong>{{ project.can_manage ? '可管理' : '只读' }}</strong></div><p>{{ project.description || '暂无项目描述' }}</p></section>
      ...
      <section class="model-table">
        <header class="model-row"><span>模型</span><span>状态</span><span>文件</span><span>添加时间</span><span /></header>
        <article v-for="model in models" :key="model.id" class="model-row">
          ...
          <div><router-link :to="`/model-projects/${projectId}/models/${model.id}`">详情</router-link><el-button v-if="model.can_manage" type="danger" link @click="removeModel(model)">删除</el-button></div>
        </article>
        <el-empty v-if="!loading && !models.length" description="该项目还没有模型" />
      </section>
    </div>
    <el-dialog v-model="editOpen" title="编辑模型项目" width="520px"><el-form label-position="top"><el-form-item label="项目名称"><el-input v-model="name" maxlength="128" /></el-form-item><el-form-item label="描述"><el-input v-model="description" type="textarea" :rows="4" maxlength="2000" /></el-form-item></el-form><template #footer><el-button @click="editOpen = false">取消</el-button><el-button type="primary" :loading="saving" :disabled="!name.trim()" @click="saveProject">保存更改</el-button></template></el-dialog>
    <el-dialog v-model="importOpen" title="导入 YOLO 模型" width="min(760px, calc(100vw - 32px))"><el-form label-position="top"><el-form-item label="模型名称"><el-input v-model="importName" maxlength="128" /></el-form-item><el-form-item label="模型描述"><el-input v-model="importDescription" type="textarea" :rows="2" maxlength="2000" /></el-form-item><el-form-item label=".pt 文件"><ServerVideoPicker v-model="sourcePath" kind="model" :allow-directory-selection="false" :allow-create="false" /></el-form-item></el-form><template #footer><el-button @click="importOpen = false">取消</el-button><el-button type="primary" :loading="importing" :disabled="!importName.trim() || !sourcePath" @click="importModel">创建导入任务</el-button></template></el-dialog>
  </main>
</template>

<style scoped>
.model-detail-page { background:#f4f7fa; color:#17212b; }.toolbar-actions,.model-row>div:last-child { display:flex; gap:10px; align-items:center; }.project-summary { margin:24px 0 16px; padding:22px; display:grid; grid-template-columns:160px 160px 1fr; gap:24px; background:#fff; border:1px solid #d8dee6; }.project-summary div { display:flex; flex-direction:column; gap:5px; }.project-summary span { color:#687482; font-size:12px; }.project-summary p { margin:0; }.model-table { margin-top:16px; background:#fff; border:1px solid #d8dee6; }.model-row { display:grid; grid-template-columns:minmax(260px,1fr) 90px 100px 110px 120px; gap:18px; align-items:center; padding:16px 20px; border-bottom:1px solid #e5e9ef; }.model-row>div:first-child { min-width:0; }.model-row code { display:block; margin-top:4px; color:#2563eb; }.model-row p { margin:4px 0 0; color:#687482; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.model-row a { color:#2563eb; text-decoration:none; }.model-table>header { color:#687482; font-size:12px; background:#f8fafc; }@media(max-width:900px){.model-row{grid-template-columns:1fr auto}.model-row>:nth-child(3),.model-row>:nth-child(4),.model-table>header{display:none}.project-summary{grid-template-columns:1fr 1fr}.project-summary p{grid-column:1/-1}}
</style>
