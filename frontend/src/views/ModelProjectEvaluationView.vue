      ...
      <template v-if="tab === 'records'">
        ...
        <VPanel flush><VTable :columns="RECORD_COLUMNS" :headers="['模型', '测试集', '格式', '状态', 'mAP50-95', '操作']">
          <VRow v-for="item in evaluations" :key="item.id" :columns="RECORD_COLUMNS">
            <div class="cell-main"><strong>{{ item.model_name }}</strong><span>{{ item.created_at.slice(0,16).replace('T',' ') }}</span></div>
            <div class="cell-main"><strong>{{ item.dataset_name }}</strong><span class="mono">{{ item.dataset_sha256.slice(0,10) }}</span></div>
