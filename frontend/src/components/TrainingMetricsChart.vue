...
<style scoped>.metrics-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.metric-card{min-width:0;border:1px solid #e0e5eb;background:#fbfcfd}.metric-card header{display:flex;justify-content:space-between;padding:12px 14px 0}.metric-card header span{color:#687482;font:12px ui-monospace,monospace}@media(max-width:1100px){.metrics-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:700px){.metrics-grid{grid-template-columns:1fr}}</style>
